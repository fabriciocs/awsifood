...
export const Order = () => {
  ...
  const orderList = useAppSelector(state => state.ifoodapp.order.entities);
  const loading = useAppSelector(state => state.ifoodapp.order.loading);
  const totalItems = useAppSelector(state => state.ifoodapp.order.totalItems);
  ...
  return (
    <div>
      <h2 id="order-heading" data-cy="OrderHeading">
        <Translate contentKey="iFoodApp.order.home.title">Orders</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="iFoodApp.order.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/order/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="iFoodApp.order.home.createLabel">Create new Order</Translate>
          </Link>
        </div>
      </h2>
        ...
        {orderList && orderList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="iFoodApp.order.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="iFoodApp.order.customer">Customer</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {orderList.map((order, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`iFoodApp.OrderStatus.${order.status}`} />
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="iFoodApp.order.home.notFound">No Orders found</Translate>
            </div>
          )
        )}
      </div>
