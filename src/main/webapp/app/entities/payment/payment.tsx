...
export const Payment = () => {
  ...
  const paymentList = useAppSelector(state => state.ifoodapp.payment.entities);
  const loading = useAppSelector(state => state.ifoodapp.payment.loading);
  ...
  return (
    <div>
      <h2 id="payment-heading" data-cy="PaymentHeading">
        <Translate contentKey="iFoodApp.payment.home.title">Payments</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="iFoodApp.payment.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/payment/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="iFoodApp.payment.home.createLabel">Create new Payment</Translate>
          </Link>
        </div>
      </h2>
        ...
        {paymentList && paymentList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="iFoodApp.payment.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('paymentDate')}>
                  <Translate contentKey="iFoodApp.payment.paymentDate">Payment Date</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('paymentDate')} />
                </th>
                <th className="hand" onClick={sort('amount')}>
                  <Translate contentKey="iFoodApp.payment.amount">Amount</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('amount')} />
                </th>
                <th className="hand" onClick={sort('paymentType')}>
                  <Translate contentKey="iFoodApp.payment.paymentType">Payment Type</Translate>{' '}
            ...
            <tbody>
              {paymentList.map((payment, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`iFoodApp.PaymentType.${payment.paymentType}`} />
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="iFoodApp.payment.home.notFound">No Payments found</Translate>
            </div>
          )
        )}
      </div>
