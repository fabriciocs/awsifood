...
export const Customer = () => {
  ...
  const customerList = useAppSelector(state => state.ifoodapp.customer.entities);
  const loading = useAppSelector(state => state.ifoodapp.customer.loading);
  const links = useAppSelector(state => state.ifoodapp.customer.links);
  const updateSuccess = useAppSelector(state => state.ifoodapp.customer.updateSuccess);
  ...
  return (
    <div>
      <h2 id="customer-heading" data-cy="CustomerHeading">
        <Translate contentKey="iFoodApp.customer.home.title">Customers</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="iFoodApp.customer.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/customer/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="iFoodApp.customer.home.createLabel">Create new Customer</Translate>
          </Link>
        </div>
      </h2>
          ...
          {customerList && customerList.length > 0 ? (
              ...
              <thead>
                <tr>
                  <th className="hand" onClick={sort('id')}>
                    <Translate contentKey="iFoodApp.customer.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                  </th>
            ...
            !loading && (
              <div className="alert alert-warning">
                <Translate contentKey="iFoodApp.customer.home.notFound">No Customers found</Translate>
              </div>
            )
          )}
        </InfiniteScroll>
